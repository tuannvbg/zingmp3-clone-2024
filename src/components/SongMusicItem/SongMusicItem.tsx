...
export default function SongMusicItem({ mymusic, playlist }: { mymusic?: boolean; playlist: SongItem[] | [] }) {
   const [isOpenModal, setIsOpenModal] = useState<boolean>(false) //tắt mở modal
   const { currentSongId, isLoadingSong, isPlaying, setAtAlbum, setIsShowLyric, setRecentSong } = useContext(AppContext)
   ...
   return (
      <>
         <div className='flex text-secondary text-xs items-center border-b border-b-gray-800 p-2.5'>
            ...
            <div className='flex-1 sm:block hidden'>{mymusic ? 'ALBUM' : 'PHÁT HÀNH'}</div>
               ...
               >
                  <div className='w-full sm:w-[60%] flex items-center gap-x-2'>
                     ...
                     <div className='flex flex-col gap-y-1'>
                        ...
                        <div className='text-xs text-secondary max-w-[150px] sm:max-w-[250px] truncate'>
                  ...
                  <div>
                     <div className='hidden sm:block'>
                        <div className='hidden group-hover:flex items-center gap-x-3'>
                           {item.mvlink && item.isWorldWide && (
                              <Tooltip content={'Xem MV'}>
                                 <Link
                                    href={item.mvlink}
                                    className={`p-2 block text-white hover:bg-white hover:bg-opacity-10 rounded-full
                            `}
                                 >
                                    <svg
                                       xmlns='http://www.w3.org/2000/svg'
